...
function setupTouchGestures() {
  ...
  // Create mode toggle button for WebXR
  const uiContainer = document.getElementById('ui-container');
  if (uiContainer && !document.getElementById('xr-toggle-mode')) {
    const btn = document.createElement('button');
    btn.id = 'xr-toggle-mode';
    btn.innerText = 'Mode: Rotate';
    btn.style.cssText = 'padding: 10px 20px; font-size: 16px; border-radius: 8px; border: none; background: #0ea5e9; color: white; margin-bottom: 20px; cursor: pointer; pointer-events: auto;';
    
    // WebXR overlays absorb events unless pointer-events is handled correctly
    btn.addEventListener('click', (e) => {
      e.stopPropagation();
      touchMode = touchMode === 'rotate' ? 'move' : 'rotate';
      btn.innerText = 'Mode: ' + (touchMode === 'rotate' ? 'Rotate' : 'Move');
    });
  ...
  dom.addEventListener('touchmove', (e) => {
    ...
    if (e.touches.length === 2 && initialDistance) {
      ...
      if (touchMode === 'rotate') {
        ...
        // Move in WebXR world space (camera relative roughly)
        islandModelGroup.position.x += deltaX * 0.005;
        islandModelGroup.position.z += deltaY * 0.005;
      }
